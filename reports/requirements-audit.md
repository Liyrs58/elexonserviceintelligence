...
The smallest close is to open `powerbi/project/Settlement.pbip` in a licensed Power BI Desktop environment, set `DataFolder`, refresh, compare the 19 measure results with `powerbi/measure-catalog.json` and the Python/SQL outputs, test slicers/interactions and capture all four pages. A licensed Fabric workspace with a supported local-file data route is the alternative. Current Azure Fabric discovery returned `UserNotLicensed`; Power BI Service is at sign-in on this Mac.

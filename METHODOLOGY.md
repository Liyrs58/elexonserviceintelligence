...
The analytical scope is one quarter and one canonical latest-message retrieval vintage. A second full-quarter retrieval reproduced all 92 daily response hashes and all analytical outputs after retrieval timestamps were excluded, but this does not extend the time window or prove that future source vintages will remain unchanged. Detailed investigation context has one retained retrieval vintage. Results are not annual seasonality, forecasts, customer billing impacts or service-availability statistics. No incident attribution is established. The official status site was inaccessible to the research fetch; that is not proof of an outage. Power BI Service returned UserNotLicensed with the existing Azure identity; live model/report verification is pending.

...
- Provide four on-canvas-labelled static design previews for Mac-side presentation review. Generate them only from the successful canonical output and the official PBIR page/visual inventory; never describe them as Power BI runtime screenshots or use them to close the runtime gate.
- Package a self-contained Windows reviewer ZIP with the PBIP project, only the CSVs required by the six model imports, checksums and the live-verification runbook. Keep raw API responses, local caches and credentials out of the bundle.
- Generate the review ZIP deterministically and replace machine-specific paths with an explicit `DataFolder` placeholder. In the packaged run receipt, retain the source-manifest identifier/hash but disclose that raw API responses are not bundled.
